...
function deltaTxt(valor, tipo = 'num') {
  const sinal = valor > 0 ? '+' : '';
  return tipo === 'pct' ? formatPP(valor) : sinal + formatInt(valor);
}
...
function anteriorFiltrado() {
  return getPeriodo(state.anterior).filter(item => state.uf === 'TODAS' || item.uf === state.uf);
}

function anteriorMap() {
  return new Map(getPeriodo(state.anterior).map(item => [item.armazem, item]));
}
...
function renderRanking(dados) {
  ...
  const max = Math.max(
    ...ordenados.map(item => state.ranking === 'estoque' ? item.estoque_loja : 1 - item.acuracia_estoque),
    1
  );
...
function renderDonut(totalAtual) {
  ...
  donut.style.background = `conic-gradient(var(--green) 0 ${percentualDevolvido}%, var(--red) ${percentualDevolvido}% 100%)`;
  legenda.innerHTML = `
    <div class="legend-item"><span><i class="dot" style="background:var(--green)"></i>Devolvido ao CD</span><b>${formatInt(totalAtual.devol)}</b></div>
    <div class="legend-item"><span><i class="dot" style="background:var(--red)"></i>Estoque em loja</span><b>${formatInt(totalAtual.estoque)}</b></div>
  `;
}
...
async function iniciarDashboard() {
  try {
    ...
    state.dados = linhas.map(normalizar).filter(item => item.armazem);
    ...
    const periodos = [...new Set(state.dados.map(item => item.periodo))]
      .sort((periodoA, periodoB) => {
        const ordemA = Math.max(...state.dados.filter(item => item.periodo === periodoA).map(item => item.ordem_mes));
        const ordemB = Math.max(...state.dados.filter(item => item.periodo === periodoB).map(item => item.ordem_mes));
